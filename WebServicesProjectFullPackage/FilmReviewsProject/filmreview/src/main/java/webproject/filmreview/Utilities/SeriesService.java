...
public class SeriesService 
{
    ...
    public List<Series> getAllSeriesFiltered(String filterType, String filterValue)
    {
        List<Series> list = new ArrayList<>();
        List<Series> allSeries = new ArrayList<>(series.values());
        switch(filterType)
        {
            case "Name":
                for(Series s : allSeries)
                {
                    if(s.getName().contains(filterValue))
                    {
                        list.add(s);
                    }
                }
